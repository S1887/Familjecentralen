...
// Sport & School ICS Feed - only sport sources + Vklass activities (no lessons)
app.get('/api/sportochskola.ics', async (req, res) => {
    console.log(`[ICS] Sport+Skola feed requested by ${req.ip} - User-Agent: ${req.get('User-Agent')}`);
    try {
        // Ensure we have data
        if (cachedCalendarEvents.length === 0) {
            console.log('[ICS] Cache empty, fetching calendars before serving feed...');
        ...
        // Add Local Events that are sport-related
        localEvents.forEach(ev => {
            if (!ev.deleted) {
                // Include if it has a sport category or came from a sport source
                const isSport = ['Bandy', 'Fotboll', 'Handboll', 'Sport'].includes(ev.category);
                const isFromSportSource = sportSourceIds.some(id => {
                    const cal = CALENDARS.find(c => c.id === id);
                    return cal && ev.source === cal.name;
                });
                if (isSport || isFromSportSource) {
                    feedEvents.push(ev);
                }
            }
        });

        // Add auto-approved sport events from cache
        cachedCalendarEvents.forEach(ev => {
            ...
            // Include Vklass activities that are NOT lessons (no isLesson flag)
            const isVklassActivity = (originCal?.id === 'vklass_skola' || originCal?.id === 'vklass_skola_tuva') && !ev.isLesson && !ev.scheduleOnly;

            if (isFromSportSource || isVklassActivity) {
                // Only include if auto-approved (not inboxOnly) OR explicitly imported
                if (!ev.inboxOnly) {
                    if (!feedEvents.find(fe => fe.uid === ev.uid)) {
                        feedEvents.push(ev);
                    }
        ...
        let icsContent = [
            'BEGIN:VCALENDAR',
            'VERSION:2.0',
            'PRODID:-//Familjecentralen//Sport och Skola v1.0//SV',
            'CALSCALE:GREGORIAN',
            'METHOD:PUBLISH',
            'X-WR-CALNAME:Sport & Skola (Örtendahls)',
            'X-WR-TIMEZONE:Europe/Stockholm',
        ];
        ...
        const foldLine = (line) => {
            ...
            start = 75;
            while (start < line.length) {
                chunks.push(' ' + line.substring(start, start + 74));
                start += 74;
            }
            return chunks.join('\r\n');
        };
        ...
        console.log(`[ICS] Sport+Skola feed generated with ${feedEvents.length} events`);
        res.set('Content-Type', 'text/calendar; charset=utf-8');
        res.send(icsContent.join('\r\n'));

    } catch (e) {
        console.error("Error generating Sport+Skola ICS feed", e);
        res.status(500).send('Error generating feed');
    }
});
